...
source("~/Desktop/solutions.R")$value
source("~/Desktop/herrings.R")$value

herrings <- data.frame(word = herrings)
solutions <- data.frame(word = solutions)

df <- data.frame(word = herrings) %>% rbind(data.frame(word = solutions))
...
word_grid <- expand.grid(answer = solutions$word, guess = df$word)
...
word_scores <- word_grid_reduced %>% 
  ...
  ungroup() %>% 
  mutate(se = sd / sqrt(n),
         upper_ci = avg + se * 1.96,
         lower_ci = avg - se * 1.96)
